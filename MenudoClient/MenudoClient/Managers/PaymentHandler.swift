...
class PaymentHandler: NSObject {
    ...
    func startPayment(products: [Product], total: Double, completion: @escaping PaymentCompletionHandler) {
        ...
        products.forEach { product in
            let item = PKPaymentSummaryItem(label: product.name, amount: NSDecimalNumber(string: String(format: "%.2f", product.price)), type: .final)
            paymentSummaryItems.append(item)
        }
        
        // Add a PKPaymentSummaryItem for the total to the paymentSummaryItems array
        let total = PKPaymentSummaryItem(label: "Total", amount: NSDecimalNumber(string: String(format: "%.2f", total)), type: .final)
        ...
        paymentRequest.merchantIdentifier = "merchant.menudo"
